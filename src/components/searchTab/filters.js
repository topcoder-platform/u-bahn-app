...
export default function SearchTabFilters({ locations, skills, achievements }) {
    ...
    const filterData = (query, initialValues, property, setState) => {
        ...
        if (q.length >= 3) {
            setState(initialValues.filter(g => g[property].toLowerCase().includes(q)));
    ...
    const getAppliedFilters = useCallback(() => {
        ...
        if (search.isFilterActive(FILTERS.AVAILABILITY)) {
            if (search.selectedAvailability.isAvailableSelected || search.selectedAvailability.isUnavailableSelected) {
                appliedFilters += 1;
            }
        }
    ...
    return (
        <div className={styles.searchTabFilters}>
                ...
                <div className={utilityStyles.mt32}>
                    <Collapsible title='Location' collapsed={false}>
                        <SearchBox
                            ...
                            onChange={q => filterData(q, locations, 'name', setLocationsData)} />
                        <TagList key='l' tags={locationsData} selected={search.selectedLocations} selector={'selectLocations'} />
                    </Collapsible>
                </div>
            }
            {
                ...
                <div className={utilityStyles.mt32}>
                    <Collapsible title='Availability' >
                        <Availability
                            availableSelected={search.selectedAvailability.isAvailableSelected}
                            unavailableSelected={search.selectedAvailability.isUnavailableSelected}
            ...
            }
            {
                ...
                <div className={utilityStyles.mt32}>
                    <Collapsible title='Skills'>
                        ...
                        <TagList tags={skillsData} selected={search.selectedSkills} selector={'selectSkills'} />
                    </Collapsible>
                </div>
            }
            {
                ...
                <div className={utilityStyles.mt32}>
                    <Collapsible title='Achievements'>
                        <SearchBox
                            ...
                            onChange={q => filterData(q, achievements, 'name', setAchievementsData)} />
                        <TagList tags={achievementsData} selected={search.selectedAchievements} selector={'selectAchievements'} />
                    </Collapsible>
                </div>
            }
            {
                ...
                <div className={utilityStyles.mt32}>
                    <Collapsible title='Project completed'>
                        ...
                        />
                        <Tabs items={tabItems} selectedIndex={0} selector={'setProjectsLevel'} />
                    </Collapsible>
                </div>
            }
            {
            ...
            {modal.isModalOpen && <EditFiltersPopup onCancel={handlePopupCancel} onDone={handlePopupDone} />}
        </div>
    );
}
        ...
        <div className={styles.searchTabFiltersSummary} >
            <div className={styles.searchTabFiltersSummaryTextContainer}>
                <div className={styles.searchTabFiltersSummaryTextContainerRow}>
                    ...
                    <div className={styles.searchTabFiltersSummaryText}>{filtersApplied} filters applied</div>
                </div>
            </div>
