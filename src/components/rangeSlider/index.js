...
const getRight = percentage => `calc(${percentage}% - ${thumbSize + thumbSize / 2}px)`;
...
export default function RangeSlider({
    ...
    const updateProgress = () => {
        const width = thumbRightRef.current.getBoundingClientRect().right - thumbLeftRef.current.getBoundingClientRect().left;
    ...
    const handleUpdateLeft = React.useCallback(
    ...
    const handleMouseMoveLeft = event => {
        ...
        const endLeft = sliderRef.current.offsetWidth - thumbLeftRef.current.offsetWidth;
        const startLeft = 0;
        ...
        if (((leftThumbLeft + thumbSize) < rightThumbLeft)
            || (direction === DIRECTIONS.LEFT && (leftThumbLeft + thumbSize) > lastPosition.current.x)) {
    ...
    const handleUpdateRight = React.useCallback(
        (valueRight, percentageRight) => {
    ...
    const handleMouseMoveRight = event => {
        ...
        const endRight = sliderRef.current.offsetWidth - thumbRightRef.current.offsetWidth;
        const startRight = 0;
        ...
        if (((leftThumbLeft + thumbSize) < rightThumbLeft)
            || (direction === DIRECTIONS.RIGHT && (leftThumbLeft + thumbSize) < lastPosition.current.x)) {
    ...
    return (
        <div className={styles.rangeSlider} ref={sliderRef}>
            ...
            <div
                ...
                ref={thumbLeftRef}
                onMouseDown={handleMouseDownLeft}></div>
            <div
                id={2}
                className={styles.rangeSliderThumb}
                ref={thumbRightRef}
                onMouseDown={handleMouseDownRight}></div>
        </div>
    );
}
